                                    ...
                                    <ul class="list list-inline mb-0">
                                        @foreach ($brands as $brand)
                                            <li class="list-item d-flex justify-content-between align-items-center">
                                                ...
                                                <span
                                                    class="text-right float-end ms-3">{{ $brand->products->count() }}</span>
                        ...
                        <div class="product-card-wrapper">
                            <div class="product-card mb-3 mb-md-4 mb-xxl-5">
                                <div class="pc__img-wrapper">
                                    ...
                                    @if (Cart::instance('cart')->content()->where('id', $product->id)->count() > 0)
                                        <a href="{{ route('cart.index') }}"
                                            class=" btn-warning mb-3 pc__atc btn anim_appear-bottom btn position-absolute border-0 text-uppercase fw-medium">Go
                                            To Cart</a>
                                    @else
                                        <form name="addtocart-form" method="post" action="{{ route('cart.add') }}">
                                            ...
                                            <button type="submit"
                                                class="pc__atc btn anim_appear-bottom btn position-absolute border-0 text-uppercase fw-medium "data-aside="cartDrawer"
                                ...
                                <div class="pc__info position-relative">
                                    ...
                                    <h6 class="pc__title"><a href="#">{{ $product->name }}</a>
                                    </h6>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            // When the 'pagesize' select element changes
            $('#pagesize').on('change', function() {
                $("#size").val($(this).val());
                console.log("Page Size Changed:", $(this).val());
                $("#frmfilter").submit();
            });

            // When the 'orderby' select element changes
            ...
            // When the checkboxes for brands change
            $("input[name='brands[]']").on("change", function() {
                var brands = ""; // Initialize an empty string for brand IDs

                // Iterate over each checked checkbox with the name 'brands'
                $("input[name='brands[]']:checked").each(function() {
                    brands += (brands === "") ? $(this).val() : "," + $(this).val();
                });

                // Update the hidden input field with the selected brand IDs
            ...
            // When the checkboxes for categories change
            $("input[name='categories[]']").on("change", function() {
                var categories = ""; // Initialize an empty string for category IDs

                // Iterate over each checked checkbox with the name 'categories'
